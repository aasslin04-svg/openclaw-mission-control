...
function formatAgo(d: string | Date, t?: any) {
  ...
  if (mins < 60) return `${mins} ${mins > 1 ? trans("minutes ago") : trans("minute ago")}`;
  if (hours < 24) return `${trans("about")} ${hours} ${hours > 1 ? trans("hours ago") : trans("hour ago")}`;
  if (days === 1) return `1 ${trans("day ago")}`;
  if (days < 7) return `${days} ${trans("days ago")}`;
      ...
      <div className="mb-3 flex items-center gap-2">
        ...
        {!valid && (
          <span className="text-xs text-amber-400">
            {t("Invalid JSON — showing raw text")}
          </span>
        )}
        {valid && mode === "view" && (
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden md:flex-row">
        ...
        <div className="flex max-h-96 w-full shrink-0 flex-col overflow-hidden border-b border-foreground/10 bg-[var(--surface-1)] md:max-h-none md:w-80 md:border-b-0 md:border-r">
          ...
          <div className="flex-1 overflow-y-auto px-2 pb-3">
            {loading ? (
              <LoadingState label={t("Loading documents...")} className="px-3 py-4 justify-start text-sm" />
        ...
        <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden bg-background/40">
          {selected ? (
            <>
              ...
              <div className="shrink-0 border-b border-foreground/10 px-4 py-4 md:px-6">
                ...
                <p className="mt-1 flex items-center gap-2 text-xs text-muted-foreground/60">
                  ...
                  {formatBytes(selected.size)} &bull; {words} {t("words")} &bull;
                  Modified {formatAgo(selected.mtime, t)} &bull;
                  {t("Use")}
                  ...
                  {t("to modify")} &bull; <SaveShortcut /> {t("to save")}
                </p>
              </div>
              ...
              <div className="flex min-h-0 flex-1 overflow-y-auto px-4 py-5 md:px-6">
                {content != null ? (
                  ...
                  <LoadingState label={t("Loading document...")} className="py-12" />
                )}
              </div>
            </>
          ) : (
            <div className="flex flex-1 flex-col items-center justify-center gap-2 text-muted-foreground/60">
              ...
              <p className="text-sm">{t("Select a document")}</p>
              <p className="text-xs text-muted-foreground/40">
                {t("Documents are grouped by agent and type")}
              </p>
            </div>
          )}
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative z-10 w-full max-w-sm rounded-2xl glass-strong animate-modal-in">
            <div className="flex items-center justify-between border-b border-foreground/10 px-5 py-4">
              <h2 className="text-sm font-bold text-foreground">{t("New Document")}</h2>
            ...
            <div className="space-y-4 px-5 py-4">
              <div>
                <label className="mb-1.5 block text-xs font-semibold text-foreground/70">{t("Workspace")}</label>
                <select
              ...
              <div>
                <label className="mb-1.5 block text-xs font-semibold text-foreground/70">{t("Filename")}</label>
                <input
                  type="text"
                ...
                />
                <p className="mt-1 text-xs text-muted-foreground/60">{t(".md extension added automatically if omitted")}</p>
              </div>
              {createError && (
